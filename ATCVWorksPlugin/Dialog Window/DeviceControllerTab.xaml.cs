...
using com.apldbio.pcr.exception;
    ...
    public partial class ATCDialog : Window
    {
        ...
        {
            string instrumentId = MainModel.DeviceTab.InstrumentId;
            if (null != instrumentId && instrumentId.Length > 0)
            {
                ...
                if (proceed)
                {
                    MainModel.DeviceTab.LoadingVisibility = Visibility.Visible;
                    IPAddress address;
                    var mode = (IPAddress.TryParse(instrumentId, out address) && (address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork)) ?
                    ...
                    {
                        MainModel.DeviceTab.LoadingVisibility = Visibility.Hidden;
                    });
        ...
        {
            MainModel.DeviceTab.LoadingVisibility = Visibility.Visible;
            Task closeConnection = Task.Run(() => DisconnectInstrument());
            Task.WaitAll(closeConnection);
            MainModel.DeviceTab.LoadingVisibility = Visibility.Hidden;
        }

        private void ConnectInstrument(string address, InstrumentControl.ConnectionMode mode)
        {
            ...
            if (ic.Connect(address, mode, new WpfDeviceControlListener(MainModel)))
            {
                InstrumentProperties properties = ic.GetInstrumentProperties();
                RunProgress progress = ic.GetRunProgress();

                // Update UI
                MainModel.DeviceTab.UpdateInstrumentProperties(properties, ic.GetAddress());
                ...
                if (null != progress)
                {
                    ...
                    {
                        MainModel.DeviceTab.UpdateRunProgress(progress.getStage(), progress.getCycle(), progress.getStep());
                    }
                }
        ...
        private void DisconnectInstrument()
        {
            ic.Disconnect();
            // Update UI
            MainModel.DeviceTab.ResetInstrumentInfo();
            MainModel.DeviceTab.ResetTemperature();
            MainModel.DeviceTab.ResetRunProgress();
        }

        private void DiscoverBtn_Click(object sender, RoutedEventArgs e)
        {
            DiscoverWindow discoverWindow = new DiscoverWindow(MainModel.DeviceTab);
            discoverWindow.ShowDialog();
        }

        private void RunTimerRadioButton_Checked(object sender, RoutedEventArgs e)
        {
            MainModel.DeviceTab.UpdateRunTimer();
        }
    }       
}
